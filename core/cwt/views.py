...
def results(request):
    ...
    if sum_user_points > (sum_user_points // 2):
        message = 'Вы молодец!'
    elif sum_user_points >= (all_points_sum//1.1):
...
def search_result(request):
    ...
    if search_q:
        ...
        # article_search = Article.objects.filter(title__icontains=search_q)
        # news_search = News.objects.filter(title__icontains=search_q
    else:
